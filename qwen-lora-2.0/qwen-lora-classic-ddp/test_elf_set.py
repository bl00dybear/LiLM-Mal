...
from utils import setup, cleanup, load_model, get_raw_model
from lilm_mal_dataset_v2 import malware_collate_fn
...
class LiLMELFDataset(torch.utils.data.Dataset):
    SYSTEM_PROMPT = (
...
def build_elf_test_loader(config, tokenizer) -> torch.utils.data.DataLoader:
    test_ds = LiLMELFDataset(tokenizer=tokenizer, config=config)
...
def eval_worker(rank, config):
    ...
    run_name = f"{model_short}-test-elf-{ratio_tag}"
    ...
    if rank == 0:
        wandb.init(
            ...
            tags=["test", "elf", ratio_tag, model_short],
        )
    ...
    try:
        ...
        model = load_model(config, rank, compile_model=False)
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                logits = model(input_ids, attention_mask, chunk_mask=chunk_mask)
